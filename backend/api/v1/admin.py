...
class AuditLogListSchema(Schema):
    ...
    event_category = fields.Str(load_default=None, validate=validate.OneOf(['authentication', 'api', 'data', 'security']))
    status = fields.Str(load_default=None, validate=validate.OneOf(['success', 'failure', 'error']))
    risk_level = fields.Str(load_default=None, validate=validate.OneOf(['low', 'medium', 'high', 'critical']))
...
class UserManagementSchema(Schema):
    ...
    is_verified = fields.Bool(load_default=None)
    is_admin = fields.Bool(load_default=None)
...
class SystemStatsSchema(Schema):
    include_user_breakdown = fields.Bool(load_default=False)
    include_processing_stats = fields.Bool(load_default=False)
    time_range_hours = fields.Int(load_default=24, validate=validate.Range(min=1, max=8760))
