...
class FaceListSchema(Schema):
    ...
    status = fields.Str(load_default=None, validate=validate.OneOf(['pending', 'processed', 'clustered', 'failed']))
    person_id = fields.Str(load_default=None)
    cluster_id = fields.Str(load_default=None)
    min_confidence = fields.Float(load_default=0.0, validate=validate.Range(min=0.0, max=1.0))
    sort_by = fields.Str(load_default='created_at', validate=validate.OneOf(['created_at', 'confidence_score', 'quality_score']))
