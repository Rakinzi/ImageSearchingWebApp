...
class ImageUploadSchema(Schema):
    files = fields.List(fields.Raw(type='file'), required=True, validate=validate.Length(min=1, max=10))
...
class ImageSearchSchema(Schema):
    ...
    similarity_threshold = fields.Float(missing=0.85, validate=validate.Range(min=0.0, max=1.0))
...
class ImageListSchema(Schema):
    ...
    status = fields.Str(missing=None, validate=validate.OneOf(['pending', 'processing', 'completed', 'failed']))
    sort_by = fields.Str(missing='created_at', validate=validate.OneOf(['created_at', 'updated_at', 'file_size', 'image_date']))
