<template>
    ...
    <div v-if="relatedImages.length" class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:grid-cols-3 ">
        <div  v-for="(image, index) in relatedImages" :key="index">
            <img  :src="BASE_URL + image" class="h-auto max-w-sm rounded-lg"  alt="">
        </div>
    </div>
  ...
  import { BASE_URL, FACES_URL } from '../stores/urls';
  ...
  const loadRelatedImages = async () => {
    try {
      const response = await axios.get(`${FACES_URL}/related_images/${faceId}`);
